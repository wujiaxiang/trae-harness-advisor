...
> **版本**: v4.1  
> **日期**: 2026-06-29  
> **变更**: v4.1 在 v4.0 基础上：①外部引用逐条核实（删伪造、修失效、标未核实）；②`.trae/specs/` 降级为完全可弃，交付物由 subagent 主动写入 `harness/` 总线（不依赖 /spec 路径）；③Contract 简化为 **Orchestrator 起 Stage 时一次标注关键点**（废多轮协商）；④board 写协议=最小更新（git 合并友好），并发=人工多对话、`depends_on` 为人工投递前的冲突规避依据；⑤补"约束强度=提示词级、非沙箱强制"措辞；⑥新增 `poc/harness-selftest/` 平台能力自检集  
...
- **`harness/` 是唯一的持久真值与跨 session 消息总线**（可 git 同步，不绑定 IDE 实现）。所有角色之间通过 `harness/` 下的文件传递状态。
- **`.trae/specs/` 完全可弃**：它只是 TRAE Work 原生 `/spec` 的临时 scratch，云端执行完即销毁、不进 git、不读取、**不依赖其路径**。
- **真正要留存的交付物由 subagent 在执行 tasklist 时主动写入 `harness/` 总线**（contract.md / gen.md / eval.md / decision.md，以及需要留存的 spec / checklist 关键信息）。我们不去"控制 /spec 的输出路径"，而是让每个角色把"我们关心的产物"写到自己的总线文件夹。
...
│   │       └── stages/{stage}/
│   │           ├── spec.md  tasks.md  checklist.md   # 持久化三件套（Orchestrator 写）
│   │           ├── contract.md          # Stage Contract（Generator↔Evaluator 协议）
│   │           └── gen.md  eval.md  decision.md      # 对抗产物（消息总线）
...
- **交付物写入总线，不依赖 `/spec` 路径**：`/spec` 在 `.trae/specs/` 下的产物视为可弃 scratch。tasklist 步骤显式要求每个 subagent 把"我们关心的交付物"写入 `harness/milestones/{milestone}/stages/{stage}/`（spec/checklist 关键信息 + contract/gen/eval/decision）。我们不去控制 `/spec` 的输出路径，而是主动把产物落到总线。
...
> **平台能力自检（已真机验证，2026-06-29）**：本设计依赖的 TRAE Work 平台假设已通过仓库的 `poc/harness-selftest/` 在真机跑过一轮，结果：
> - **已验证 PASS**：stage-executor 按触发短语**自动加载**（AP1）；SubAgent 加载指定角色 Skill（AP2，gen/eval 确由两个独立 Task 子代理分别加载 generator-role/evaluator-role 写入）；**子代理间上下文隔离**（AP3，Evaluator 拿不到 Generator 上下文，只能读 gen.md）；路径白名单**提示词级但被遵守**（AP5，拒绝写 `/etc/hosts`）；产物写入 `harness/` 总线（AP6）；原生 checklist=完成性 gate（AP7）；RULE.md 钩子（AP8）；SubAgent **串行派发 + 无自动循环**（AP9 的一半）。
> - **未满足 / 待补证**：**MCP 全平台未注册**（AP4，连主 Agent 都无 `mcp__` 工具，需配 MCP server 后重测；故 `verification_mode=full` 的浏览器验证在该环境降级）；SubAgent **真并行**未实证（AP9 另一半，本次实为串行）。
> - **设计补注**：本次 **[DECISION] 由主 Orchestrator 自己执行**且能看到子代理 Task 返回摘要，故"中立裁决者"非盲审第三方——若需真正中立，应把 Decision 也改为独立 SubAgent（见 3.6 待定项）。
> 详见 `poc/harness-selftest/expected-outcome.md`。
...
|------|------|------|------|----------------|-------------|
| L0 | **Advisor Skill** | 用户一次性调用 | 技术栈、目录偏好、TDD、严格度等 | 1. 角色 Skill（planner/generator/evaluator-role）<br>2. **stage-executor** playbook Skill<br>3. RULE.md + 钩子规则文本<br>4. `harness/templates/*.skeleton.md`（三件套+Contract 骨架）<br>5. 空 `state-board.json`<br>6. 可选 Agent 配置 | 据技术栈定制编码规范、TDD 流程、评估维度 |
...
| 执行层 | **Decision** | Evaluator 完成 | gen.md + eval.md | `decision.md`（pass/retry/escalate） | 中立裁决 |
...
**实现方式说明**：TRAE Work 云端唯一自动加载的配置通道是 `.trae/skills/`。`.trae/agents/` 当前不支持，因此 G/E/D 的工具集、路径白名单、Decision 裁决规则**内嵌到对应 Skill** 中；Agent 配置文件保留为可选生成，供未来兼容。
...
- **Orchestrator 是运行时执行者**：读 board 定位 Stage → `/spec` 产三件套 → 持久化 `harness/` → 自检 → 派发 G/E/D → 回写 board。
- **Decision 是裁决代理**：只读 gen.md + eval.md → 输出裁决。pass/retry 自动处理，escalate 升级人类。
...
> **简化的 Contract（v4.1）**：不再让 Generator↔Evaluator 多轮协商 Contract。改由 **Orchestrator（主 Agent）在起 Stage 时直接标注关键 Contract 点**（本轮目标、可验收要点、边界）写入 `contract.md`；Generator 据此实现，Evaluator 据此验收。这把"协商"压成"一次标注"，降低复杂度。若 `force_contract=false`，则跳过标注，Generator 直接按 spec 实现。
...
**输出边界**：Planner 输出两样：
1. **`milestone-plan.md`**：Milestone 概述（含 `kind: development | verification`）+ 每个 Stage 的定义（目标、验收标准要点、`depends_on` 依赖、预估复杂度）
...
> 取舍：若要 Decision 真正中立，应把它也实现为**独立 SubAgent**（只 Read gen.md/eval.md/contract.md，无双方对话上下文）。代价是多一次 SubAgent 派发。若可接受"Orchestrator 兼任裁决"（更省、但弱中立），则维持现状。**默认建议**：高严格度/对抗性强的项目用独立 Decision SubAgent；轻量项目可由 Orchestrator 兼任。
...
- `harness/milestones/{milestone}/stages/{stage}/spec.md|tasks.md|checklist.md` — 当前 Stage 三件套（交付物写入总线）
...
```
Orchestrator 产出 spec.md（当前 Stage）
        │
        ▼
Orchestrator 标注关键 Contract 点 → contract.md（目标 / 验收要点 / 边界）
        │
...
│   └── stages/{stage}/
│       ├── spec.md  tasks.md  checklist.md   # Orchestrator 运行时产出
│       ├── contract.md          # Stage Contract
│       └── gen.md  eval.md  decision.md      # G/E/D 产物
└── state-board.json             # 状态机真值
.trae/specs/                     # 原生临时 scratch（gitignore，不依赖）
```
...
| 2 | Orchestrator | milestone-plan + 骨架 | 三件套 spec/tasks/checklist → harness/ | stage-executor playbook，每 Stage 一次 /spec |
| 3 | Orchestrator | spec.md | 标注关键 Contract 点 → contract.md | 目标/验收要点/边界（一次标注，非多轮协商） |
...
| 7 | Decision | gen.md + eval.md | pass/retry/escalate → decision.md | 裁决（rounds≤3，超限 escalate） |
| 8 | Orchestrator | checklist.md | 完成性 gate 通过 → 回写 board | 底层机制完成性验收 |

**Stage Contract 文件模板**（由 Orchestrator 标注关键点，非多轮协商）：
...
Planner 产出 milestone-plan.md（含 Stage 定义）后，人类确认。之后逐个 Stage：Orchestrator 经 stage-executor playbook 启动，按骨架产出三件套（spec/tasks/checklist）持久化到 harness/，tasks.md 含 [GENERATOR]/[EVALUATOR]/[DECISION] 标记的顺序对抗步骤。
