...
> **版本**: v4.6  
> **日期**: 2026-07-02  
> **变更**: v4.6 **三档自动化 + Operator 操作员角色**：把原「人节点」拆成「操作员（机械搬运）+ 监督者（判断）」，给出三档自动化供用户自选——A 人（现状）/ B 人+Codex-CUA（变相全自动，机器分批替代人分批，fanout/tournament/generate-filter 收益最大）/ C 父 agent 走 TRAE API（依赖平台）。新增**本框架唯一运行在 TRAE Work 之外**的可选角色 `operator-playbook.md`（`generate_operator=true`，含护栏/调度循环/监督者交接清单/CUA 与 ToS 风险），**不放进 `.trae/skills/`**；escalate/纠偏/授权永远归监督者，B/C 档只改变「谁点鼠标」不改变「谁拍板」。详见 §1.4.5 与 deliverable-specs §11b  
> **v4.5**: **多模式编排框架（6 种编排模式全做）**：基于 v4.4 已真机验证的 Orchestrator 图灵完备底座 + 独立 SubAgent + harness 总线，把 Claude Code Dynamic Workflows 的 6 种模式全部用同一套原语落地——新增 3 个轻量角色（Classifier/Synthesizer/Selector）+ 4 个 pattern playbook（classify/fanout/generate-filter/tournament），Stage 层新增 `pattern` 字段由 `stage-executor` 路由；**6 种模式均已真机端到端验证**（adversarial/loop=AP1–AP14；classify/fanout/generate-filter/tournament=AP15–AP18，commit c9a5e84，pattern 路由链路 + 3 新角色加载全通过）；新增 `generate_patterns` 生成开关（默认 false，见 §3.10 与 deliverable-specs §11）  
...
#### 1.4.5 三档自动化：把"人节点"拆成操作员 + 监督者

1.4.4 说的"启动钥匙在人手里"，其实混了两类活。把它拆开后，人真正不可替代的只有一半：

| 子职责 | 性质 | 谁干 |
|---|---|---|
| **操作员工作**：开新对话、贴固定提示、点"云端运行"、等完成、读 `decision.md`/`state-board.json` 推进下一个 Stage | 机械、可脚本化、重复 | 可外包 |
| **监督者工作**：是否接受 escalate、纠偏方向、给 API Key/授权、跨 Milestone 取舍 | 判断、不可外包 | 必须人（或强 LLM 父 agent） |

据此给出**三档自动化**，让用户按介入预算与风险偏好自选（谁来扮演"操作员"）：

| 档 | 扮演操作员 | 效果 | 代价 / 前提 |
|---|---|---|---|
| **A** | 人 | 现状：人兼操作员+监督者 | 每个 Stage 手动开对话；适合低频、重决策 |
| **B** | **Codex / CUA（计算机使用代理）** | **变相全自动**：机器搬运，人只监督/纠偏 | 需监督预算；受 CUA 可靠性、成本、平台 ToS 约束 |
| **C** | 强 LLM 父 agent 走 TRAE API | 最干净：API 编排，无屏幕驱动 | **依赖 TRAE 暴露 API/Web 控制台**，当前多为平台限制 |

**B 档的现实边界（客观）**：CUA 端到端驱动复杂桌面任务的成功率仍偏低（业界 OSWorld ~38%）、延迟以分钟计、随 UI 改版易失效；但浏览器原生任务可靠得多（WebVoyager ~87%）。因此 B 档要把 CUA 的脆弱面**压到最小**——只做"触发云端运行 + 贴提示 + 取回产物路径"这个不可省的 UI 动作，其余状态交换全部走 git/`harness/` 总线文本（读 board，别靠截图解析业务状态）。并用我们自己的 PGE 思路管这个会犯错的操作员：动作幂等、有界重试、事件驱动、失败即上抛人。

**收益随模式不同**：Operator 的价值 = 该模式需要重复开对话/推进的次数。fanout / tournament / generate-filter 收益最大——之前"超大 fan-out/tournament 受上下文预算需**人**分批"这条限制，B 档把它变成"**机器**分批"；adversarial/loop 本就单窗口自动化，收益有限。

这套外部调度逻辑固化为一个可选角色 **Operator playbook**（`{harness_dir}operator-playbook.md`，`generate_operator=true` 时生成）——**本框架唯一运行在 TRAE Work 之外**的角色，含护栏、调度循环、监督者交接清单、逐模式操作要点与 CUA/ToS 风险提示。**它不放进 `.trae/skills/`**（那是 TRAE Work 内部技能目录），而是总线里供人/CUA/父agent 消费的说明书。

> 一句话：B/C 档并不改变"谁拍板"——escalate/纠偏/授权永远归监督者；它们只改变"谁点鼠标"，把人从无谓的机械搬运里解放出来，只在真正的决策与纠偏处介入。
...
|------|------|------|------|----------------|-------------|
| L0 | **Advisor Skill** | 用户一次性调用 | 技术栈、目录偏好、TDD、严格度等 | 1. 角色 Skill（planner/generator/evaluator/decision-role）<br>2. **stage-executor** playbook Skill<br>3. RULE.md + 钩子规则文本<br>4. `harness/templates/*.skeleton.md`（三件套+Contract 骨架）<br>5. 空 `state-board.json`<br>6. `harness/references/llm-task-authoring-best-practices.md`（共享方法论）<br>7. 可选 Agent 配置<br>8. 可选多模式包（`generate_patterns`：3 角色+4 playbook）<br>9. 可选 `harness/operator-playbook.md`（`generate_operator`，三档自动化 B 档） | 据技术栈定制编码规范、TDD 流程、评估维度 |
...
**自动化程度无法追齐**（指平台默认能力；引入外部 Operator / 父 agent 可提升，见 §1.4.5 三档自动化）。核心差距在于：
