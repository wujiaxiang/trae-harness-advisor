...
> **版本**: v4.3  
> **日期**: 2026-06-29  
> **变更**: v4.3 验收标准来源澄清 + 可选共识子阶段：①明确 contract.md 的"根"在 Planner 的 `milestone-plan.md`（每 Stage 带验收标准要点），Orchestrator 只誊写/收敛、不凭空发明；②每个 Stage 标 **`contract_mode: planned | codraft`**——`planned`（默认，需求清晰/联调，Orchestrator 直接写 contract.md）、`codraft`（可选，验收标准需先有草稿才能定 → Generator 草稿+提议标准 → Evaluator 敲定 → 写 contract.md，再对抗）  
