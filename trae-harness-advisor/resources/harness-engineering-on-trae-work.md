...
> **平台能力自检（先验证再铺开）**：本设计依赖几条 TRAE Work 平台假设（SubAgent 能否加载指定 Skill、能否调 MCP、白名单是否生效、产物能否写入 `harness/` 总线、原生 checklist 语义）。这些为 **ASSUMPTION（待 PoC）**。仓库提供了最小自检集 `poc/harness-selftest/`——喂给真机 TRAE Work 跑一遍，按 `expected-outcome.md` 判读后再决定是否大规模铺开。
