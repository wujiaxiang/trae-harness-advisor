...
> **版本**: v4.2  
> **日期**: 2026-06-29  
> **变更**: v4.2 基于真机自检结果：①**Decision 改为独立 SubAgent**（decision-role），从 evaluator-role 抽出，与 G/E 上下文隔离保证中立盲审；Orchestrator **只串联流程、不兼任任何角色**；②**retry 闭环**明确：Orchestrator 收到 retry 后可**改 tasks.md + 重派 Generator**（新增自检点 AP10）；③核心 Skill 4→5、核心文件 10→11；④真机自检 AP1/2/3/5/6/7/8 已硬验证 PASS、AP9 串行+无循环已证（并行待补）、AP4 MCP 全平台未注册（待配置补证）  
...
> - **设计补注 → 已修正（v4.2）**：首轮 **[DECISION] 由主 Orchestrator 自己执行**且能看到子代理摘要，故非盲审。**已改为：Decision 一律独立 SubAgent（decision-role），Orchestrator 只串流程不兼任裁决**（见 3.2/3.6）。retry 时由 Orchestrator 改 tasks.md + 重派 Generator（AP10 待补测）。
