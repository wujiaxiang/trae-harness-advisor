...
> **平台能力自检（已真机验证，v4.3 重跑）**：`poc/harness-selftest/` 在真实 TRAE Work 跑通两轮，结论：
> - **已验证 PASS**：stage-executor 触发短语**自动加载**（AP1）；SubAgent 加载指定角色 Skill（AP2，含独立 decision-role）；**G/E/D 三方上下文隔离**（AP3，各自只读总线文件）；路径白名单提示词级但被遵守（AP5）；交付物写 `harness/`、三件套留 `.trae/specs/`（AP6）；原生 checklist=完成性 gate（AP7）；RULE.md 钩子（AP8）；SubAgent **真并行 + 无自动循环**（AP9）；**retry 闭环**——Orchestrator 改 tasks.md + 手动重派一轮（AP10）。
> - **决定性 FAIL（有设计影响）**：**SubAgent 不继承 MCP**（AP4）——即便在 TRAE Work 配置并启用了 Playwright MCP，`mcp__*` 工具**只对主 Orchestrator 可见，不下发给 SubAgent**。因此 **Evaluator 子代理无法用 MCP 做浏览器验证**；`verification_mode=full` 的浏览器步骤须由**主 Orchestrator 代行**（它有 MCP），把证据（截图/日志）写入总线供 Evaluator 评分，或将默认 verification_mode 降级为 `automated`（见 3.5 Evaluator 说明）。
