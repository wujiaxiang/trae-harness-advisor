...
> **平台能力自检（先验证再铺开）**：本设计依赖几条 TRAE Work 平台假设——Skill 自动加载、SubAgent 加载指定角色 Skill、上下文隔离、能否调 MCP、白名单是否生效、产物能否写入 `harness/` 总线、原生 checklist 语义、RULE.md 钩子。这些为 **ASSUMPTION（待 PoC）**。仓库已**实例化**一套自检环境（`.trae/skills/`+`RULE.md`+`harness/`）并提供测试套件 `poc/harness-selftest/`——按其 `test-prompt.md` 喂给真机 TRAE Work 跑一遍，对照 `expected-outcome.md`（AP1–AP8）判读后再决定是否大规模铺开。
