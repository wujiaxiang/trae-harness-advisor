...
- `evaluator_shell_bridge`（实验，需 AP19 真机验证）：TRAE Work 远程环境 install 阶段初始化 `harness/mcp-bridge/`；Stage Orchestrator 只运行 `check.sh --json` 并把白名单能力写入 contract；Evaluator 在自己的 SubAgent 上下文内通过 shell bridge 查证，证据写入 `eval.md`。禁止自由扫描未知 MCP。
