...
> **版本**: v4.0  
> **日期**: 2026-06-29  
> **变更**: v4.0 概念重构——引入 **Milestone / Stage / Task** 三级层次严格定义（弃用 feature、sprint）；SPEC 三件套下沉到 **Stage 层**由 Orchestrator 运行时创建；新增 **stage-executor playbook** 单一拉起入口；checklist（底层机制：Trae 原生完成性 gate）与 Evaluator（业务质量：我们编排、在 task 内部运行的对抗验收）分工去重；对抗循环改为**顺序模拟、最多 3 轮、超限 escalate**；状态模型重构为 `state-board.json` v2（milestone.kind + stage.depends_on + 状态机）；持久产物与消息总线统一落在 `harness/`，`.trae/specs/` 仅作原生临时 scratch  
...
- **`.trae/specs/` 仅为 TRAE Work 原生 `/spec` 的临时 scratch**：云端执行完即销毁、不进 git、**不用于核心消息传递**。Orchestrator 必须把三件套**持久化到 `harness/`** 作为真值，不得依赖 `.trae/specs/`。
...
- 产出后须**持久化到 `harness/milestones/{milestone}/stages/{stage}/`**；`.trae/specs/` 的副本视为临时 scratch。
...
> 调研结论：TRAE Work 的 Task 下 SubAgent 只能**顺序执行**，没有真实控制流循环。因此对抗是在**同一个 Stage 对话内顺序模拟**的，由 Orchestrator 按下述步骤逐步派发，最多 3 轮返工，超限即 escalate 人工介入。
...
- 产品概述占位符
- 技术栈占位符
- Stage 分解占位符（每个 Stage 的目标、验收标准、依赖关系）
- 非功能性需求占位符（量化指标）
- 开放问题占位符

Orchestrator 启动后读取 spec.md 模板 + milestone-plan.md（全局规划），按照 Harness 编排模式推理填充 spec.md → 生成 tasks.md。Planner 不干预Orchestrator 的编排决策。

**Planner 的核心原则**：

1. 从用户需求中提取核心功能，而非技术实现细节
2. 将功能分解为独立、可验证的 Stage，每个 Stage 有明确的验收标准
3. 识别 Stage 之间的依赖关系，标注必须串行的强依赖
4. 验收标准必须是 Evaluator 可以机械检查的，而非主观判断

### 3.4 Generator 角色定义

**职责**：根据 Planner 的规格说明，以 Stage 为单位实现功能。只负责构建，不负责评判。

**核心行为**：

1. 读取 Planner 输出的 spec.md 和 tasks.md
2. 对当前 Stage 提出 Contract 草案（"本轮构建什么、如何验证成功"）
3. 与 Evaluator 协商 Contract 直到达成一致
4. 严格遵循 TDD：先写测试，确认测试失败，再写实现
5. 每次代码改动后立即运行测试
6. 完成一个 Stage 后立即 git commit
7. 将实现总结写入 `harness/milestones/{milestone}/stages/{stage}/gen.md`
...
- `harness/milestones/{milestone}/stages/{stage}/spec.md|tasks.md|checklist.md` — Orchestrator 产出的三件套
- `harness/milestones/{milestone}/stages/{stage}/contract.md` — Stage Contract 协议文件
...
Orchestrator 产出 spec.md（当前 Stage）
        │
        ▼
Generator 提出 Stage Contract 草案
        │
        ▼
...
| 3 | Generator | spec.md, tasks.md | Stage Contract 草案 → contract.md | 提出本轮构建内容 |
| 4 | Evaluator | Contract 草案 | 审查意见 | 最多 3 轮迭代 |
| 5 | 双方 | 审查意见 | 达成一致的 Contract | 写入 contract.md |
| 6 | Generator | Contract | 代码实现 + 测试 | TDD 驱动 |
| 7 | Generator | 代码 | 自检 + git commit | 实现总结 → gen.md |
| 8 | Evaluator | 代码 + Contract | 业务质量评估 → eval.md | 四维评分（task 内部） |
| 9 | Decision | gen.md + eval.md | pass/retry/escalate → decision.md | 裁决（rounds≤3，超限 escalate） |
| 10 | Orchestrator | checklist.md | 完成性 gate 通过 → 回写 board | 底层机制完成性验收 |
...
Generator 读取 `spec.md` 和 `tasks.md`，提出 Stage 1 的 Contract 草案。Evaluator 审查后提出修改意见：增加 Alembic 迁移脚本要求、增加数据库连接错误处理。双方达成一致后，Generator 开始编码。
