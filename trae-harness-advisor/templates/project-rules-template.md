...
- src/ — 源代码
- tests/ — 测试文件
- {harness_dir} — 持久真值与消息总线（milestone-plan、contract、gen/eval/decision、browser-check、state-board.json）
- {harness_dir}templates/ — 三件套与 Stage Contract 的结构骨架，只含章节契约，无业务内容
- {harness_dir}milestones/{milestone}/stages/{stage}/ — Stage 级持久产物目录
- .trae/specs/ — 原生 /spec 临时 scratch（gitignored，不依赖，不做消息传递）
- {skill_dir} — 角色 Skill 与 stage-orchestrator playbook（静态配置，git 同步）
- {agent_dir} — 可选 Agent 配置（未来兼容）
...
## 全局禁止修改
- {harness_dir}（除 Orchestrator 回写状态、contract/browser-check 与 gen/eval/decision 产物外）
- {skill_dir}
- RULE.md
- node_modules/
- .git/
