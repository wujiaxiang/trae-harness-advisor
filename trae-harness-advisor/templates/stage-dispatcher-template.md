...
> 这不是 TRAE Work 内部 SubAgent 技能，**不放进 `{skill_dir}`**。它是一份放在总线里的外部派发说明（`{harness_dir}stage-dispatcher.md`），供**人照着做**、或**喂给 Codex/CUA 当机械执行指令**、或**给父 agent 当编排脚本**。
...
1. 读 `{harness_dir}state-board.json` 和 `milestone-plan.md`，找出 `status=planned` 且依赖已通过的 Stage。
...
6. 追加 `{harness_dir}stage-dispatcher-log.md`。
...
2. **只读持久总线**：跨 session 状态只读 `{harness_dir}`；`.trae/specs/` 是 Stage 对话内 scratch，不是 Dispatcher 真值来源。
...
- `{harness_dir}stage-dispatcher-log.md`：唯一可写产物，追加式调度审计日志。
