...
- **evaluator_shell_bridge（实验增强，需 AP19 真机验证）**：运行 `{harness_dir}mcp-bridge/check.sh --json`，读取 `config/mcporter.json`，把可用白名单命令和 MCP→Shell 翻译表写入 `contract.md` 的 `mcp_bridge_capabilities` / `mcp_to_shell_translation`。bridge 可用时不要代行浏览器观察；Evaluator 在自己的 SubAgent 上下文内按翻译表把 MCP/browser 意图改写成 RunCommand，并把证据写入 `eval.md`。bridge 不可用时，按 contract 策略 fallback 到 `orchestrator_delegated`，或暂停并要求 `[BLOCKED: MCP bridge unavailable]`。
