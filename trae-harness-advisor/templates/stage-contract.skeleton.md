...
- mcp_to_shell_translation:
  - 当你想使用 MCP/browser 的 `navigate` 能力时，改用：`{harness_dir}mcp-bridge/bin/mcp-browser navigate {url}`
  - 当你想使用 MCP/browser 的 `snapshot` 能力时，改用：`{harness_dir}mcp-bridge/bin/mcp-browser snapshot --output {harness_dir}milestones/{milestone}/stages/{stage}/evidence/browser-snapshot.json`
  - 当你想使用 MCP/browser 的 `screenshot` 能力时，改用：`{harness_dir}mcp-bridge/bin/mcp-browser screenshot --output {harness_dir}milestones/{milestone}/stages/{stage}/evidence/screenshot.png`
  - 当你想读取页面文字时，改用：`{harness_dir}mcp-bridge/bin/mcp-browser text`
