...
5d. 浏览器/外部验证的 MCP 访问模式？
   ...
   B. evaluator_shell_bridge（实验）— 额外生成 harness/mcp-bridge/*；远程环境 install 必须运行
      `cd /workspace && bash harness/mcp-bridge/install.sh`（或仓库实际 clone 目录）。Evaluator 只能使用 contract.md 声明的白名单 shell bridge
