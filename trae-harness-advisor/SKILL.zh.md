...
- 用户想要生成角色 Skill、stage-orchestrator playbook、RULE.md 或三件套骨架

**以下情况请勿使用本 Skill：**
- 用户只是询问 Harness Engineering 的理论问题（直接回答即可）
- 用户想要执行某个 Stage（使用生成出的 stage-orchestrator playbook，而非本 Skill）
...
输入:
  ...
  - generate_stage_dispatcher: 布尔值（默认: false；开启则额外生成 Stage Dispatcher 文件——执行阶段外部机械派发器，见 deliverable-specs 第 11b 节）
  - mcp_access_mode: "orchestrator_delegated" | "evaluator_shell_bridge"（默认: "orchestrator_delegated"；shell bridge 为实验能力，必须通过 AP19 真机验证）

输出（核心 12 个文件：11 个权威文件 + 1 个兼容 shim）:
  ...
  - {skill_dir}stage-orchestrator/SKILL.md   # 运行时拉起 playbook（L2 单一入口，只串联不兼任角色）
  - {skill_dir}stage-executor/SKILL.md       # 旧名兼容 shim
...
```
4. 持久产物根目录（harness/，存放 milestone-plan、contract、gen/eval/decision、browser-check、state-board；三件套留 .trae/specs 临时区）？
...
5b. 是否生成多模式编排包（默认 adversarial/PGE 之外的其它模式）？
   A. 不需要（默认）— 只用 adversarial + loop（已内置于 stage-orchestrator），每个 Stage 走 adversarial
   B. 需要 — 额外生成 6 种模式包：3 个轻量角色（Classifier/Synthesizer/Selector）
      ...
      标 `pattern` 字段，stage-orchestrator 据此路由（见 deliverable-specs 第 11 节）

5c. 是否生成 Stage Dispatcher 文件（三档自动化的 B 档：把执行阶段派发交给机器）？
   A. 不需要（默认）— A 档：由人手动打开每个 Stage 执行对话
   B. 需要 — 额外生成 stage-dispatcher.md。它只做机械执行派发
      （读 board → 打开 TRAE Work 执行对话 → 调用 @stage-orchestrator → 读 decision → 推进或上抛），
      规划确认、review、授权、业务取舍和最终仲裁仍归人类 Supervisor/Lead。
      在 fanout/tournament/generate-filter 收益最大。见 deliverable-specs 第 11b 节。

5d. 浏览器/外部验证的 MCP 访问模式？
   A. orchestrator_delegated（默认）— root Stage Orchestrator 使用 MCP，并写 browser-check.md
   B. evaluator_shell_bridge（实验）— 使用由 trae-mcp-bridge-advisor 维护的现有 bridge；远程环境 install 必须运行
      `cd /workspace && bash tools/mcp-bridge/install.sh`（或仓库实际 clone 目录）。Evaluator 只能使用 contract.md 声明的白名单 shell bridge
      命令自查，并把证据写入 eval.md。稳定使用前必须通过 AP19 真机验证。
```
...
6. 每个 Stage 最多进行几轮对抗返工？
   A. 默认: 3 轮（动态编排对抗，超限则 escalate 人工介入）
   B. 自定义: 你来指定数字
...
6. stage-orchestrator playbook → {skill_dir}stage-orchestrator/SKILL.md（只串联，不兼任角色）
6b. stage-executor shim        → {skill_dir}stage-executor/SKILL.md（旧名兼容）
7. RULE.md（根目录）           → 编码规范 + 禁止修改路径 + 指向 stage-orchestrator
...
15.（可选，mcp_access_mode=evaluator_shell_bridge）MCP bridge 外部依赖（实验 AP19，见 deliverable-specs 第 11c 节）：
    → 本 Skill 不生成 bridge runtime
    → 检查 config/mcporter.json + tools/mcp-bridge/{install.sh,check.sh}
    → 若缺失，输出 [BLOCKED: MCP bridge not installed] 并提示使用 trae-mcp-bridge-advisor
```
...
2. 下一步操作指引：
   ...
   - 逐个 Stage：触发 stage-orchestrator playbook，Orchestrator 按骨架产三件套、顺序派发 G→E→D（最多 {max_rounds} 轮，超限 escalate）
