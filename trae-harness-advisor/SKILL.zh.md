...
输入:
  ...
  - generate_operator: 布尔值（默认: false；开启则额外生成 Operator playbook——本框架唯一运行在 TRAE Work **之外**的角色，由人 / Codex-CUA / 父 agent 扮演，仅「三档自动化」B 档需要，见 deliverable-specs 第 11b 节）
...
5c. 是否生成 Operator playbook（三档自动化的 B 档：把跨 Stage 调度交给机器）？
   A. 不需要（默认）— A 档：由人当操作员，手动开每个 Stage 的对话
   B. 需要 — 额外生成 operator-playbook.md：本框架唯一运行在 TRAE Work **之外**的角色，由人 /
      Codex-CUA / 父 agent 扮演，机械调度 Stage（读 board → 在 TRAE Work 派发 → 读 decision → 推进），
      而一切判断（接受 escalate、纠偏、给授权）仍上抛人类监督者。在 fanout/tournament/generate-filter
      收益最大（机器分批替代人分批）。见 deliverable-specs 第 11b 节。
```
...
14.（可选，generate_operator=true）Operator playbook（三档自动化 B 档，见 deliverable-specs 第 11b 节）：
    → {harness_dir}operator-playbook.md   （唯一运行在 TRAE Work 之外的角色；**不放进** {skill_dir}）
```
