...
> 本文是 `trae-harness-advisor` Skill 的方法论参考文件（v4.2）。
...
端到端 8 步（步骤 1 为 Planner 规划、2 为 Orchestrator 起 Stage、8 为完成性 gate；步骤 3-7 是单 Stage 的**顺序模拟对抗**，最多 3 轮返工，超限 escalate）：
...
3. Orchestrator 标注关键 Contract 点 → contract.md（目标/验收要点/边界，一次标注非协商）
...
> 调研：TRAE Work 的 SubAgent **可并行可串行派发，但无自动控制流循环（loop）**；Stage 内 G→E→D 因数据依赖串行，多轮返工靠每轮手动重新派发（非自动循环）。
