...
| **Orchestrator** | 每 Stage 运行时产三件套（留 .trae/specs）+ 派发 | 交付物 contract/gen/eval/decision 写 harness/ + 回写 board | stage-executor playbook |
...
多模式包额外生成 3 个轻量角色（Classifier/Synthesizer/Selector）+ 4 个 pattern playbook；6 种模式均已真机端到端验证（AP1–AP18）。
...
把原"人节点"拆成**操作员**（开对话/贴提示/读 decision 推进——机械、可外包）+ **监督者**（接受 escalate/纠偏/给授权——判断、不可外包）。据此三档自选：

| 档 | 操作员 | 效果 | 前提 |
|---|---|---|---|
| **A** | 人 | 现状：人兼操作员+监督者 | 低频、重决策 |
| **B** | Codex/CUA | 变相全自动，机器搬运人只监督 | 受 CUA 可靠性/成本/平台 ToS 约束 |
| **C** | 强 LLM 父 agent 走 TRAE API | 最干净，API 编排 | 依赖平台暴露 API |

B 档收益随模式不同（fanout/tournament/generate-filter 最大——机器分批替代人分批）。逻辑固化为可选角色 **Operator playbook**（`{harness_dir}operator-playbook.md`，`generate_operator=true`）——**本框架唯一运行在 TRAE Work 之外**的角色，**不放进 `.trae/skills/`**。无论哪档，escalate/纠偏/授权永远归监督者。详见主文档 §1.4.5。
