...
4. **补测 AP19**：云端运行环境 install 需执行 `cd /workspace && bash tools/mcp-bridge/install.sh`（或仓库实际 clone 目录）；MCP 安装和 wrapper 白名单由 `config/mcporter.json` 维护。复制 **第 1c 步**那段跑 Evaluator shell bridge 验证。
5. 跑完让它把 `VERIFY[APn]` 汇总成表并 push 到 main。
