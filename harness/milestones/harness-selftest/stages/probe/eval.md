...
## AP19 实验补测（evaluator_shell_bridge）

> 独立 Evaluator SubAgent 在自己上下文内按 contract.md「mcp_to_shell_translation」把 MCP/browser 意图改写成白名单 shell 命令查证。Orchestrator 不代行浏览器中间观察。本段不改变 AP1–AP11 已验证结论。

- `VERIFY[AP19]: PASS — shell bridge wrapper 真实路由到 mcporter call/playwright server（返回 isError=true 的结构化错误），浏览器动作因 chrome 二进制未预装失败但 bridge 机制本身已通；Evaluator 已通过白名单 shell 完成一次查证并落盘 eval.md。`

### 原始 MCP 意图
1. `playwright.browser_navigate`（导航到 https://example.com）
2. `playwright.browser_snapshot`（捕获浏览器快照取证）

### 实际执行的 shell 命令（完整命令行，均经 contract 白名单 wrapper）
1. `bash harness/mcp-bridge/bin/mcp-browser --bridge-check`（cwd=/workspace，daemon 存活自检）
2. `bash harness/mcp-bridge/bin/mcp-browser playwright.browser_navigate url:https://example.com`（cwd=/workspace）
3. `bash harness/mcp-bridge/bin/mcp-browser playwright.browser_snapshot`（cwd=/workspace）

### 关键输出摘要

**1. bridge-check（daemon 存活自检）**
```
$ bash harness/mcp-bridge/bin/mcp-browser --bridge-check 2>&1
（stdout/stderr 无输出）
=== exit=0 ===
```
退出码 0，wrapper 路由成功，未报 `[BLOCKED: MCP bridge unavailable]` 也未报 `[BLOCKED: MCP bridge command not allowed]`，与 contract「bridge 自检结果 available=true」一致。

**2. navigate（导航到 example.com）**
```json
{
  "content": [
    {
...
### 截图/snapshot/trace 路径，或 BLOCKED 原因

- 无截图/snapshot/trace 文件落盘：浏览器动作因 chrome 二进制未预装（`/opt/google/chrome/chrome` 不存在）失败，未能进入页面渲染阶段，与 AP11 browser-check.md 记录的 chromium 二进制缺失属同一环境前置问题。
- 非 BLOCKED：bridge wrapper 自检退出码 0、未输出 `[BLOCKED: MCP bridge unavailable]`、未输出 `[BLOCKED: MCP bridge command not allowed]`，bridge 链路本身可用，不触发 contract 末段「bridge 不可用则 BLOCKED」分支（本次 available=true）。

### PASS|FAIL|BLOCKED 结论与一句话理由

**结论：PASS**

理由：依据 contract.md「AP19 实验补测」末段——「如果 bridge wrapper 执行了但浏览器二进制缺失导致浏览器动作失败，bridge 机制本身仍算通（wrapper 路由到 mcporter call 了）」——本次 wrapper 真实路由到 mcporter call（返回了 playwright server 的结构化 `isError:true` 响应，而非 wrapper 自身的 BLOCKED 拦截），浏览器动作失败根因为 chrome 二进制环境前置缺失（与 AP11 同源，不作为 bridge 机制扣分项）；同时 contract 已含 `mcp_bridge_capabilities` + `mcp_to_shell_translation`，Evaluator 已在自己上下文内按翻译表通过白名单 shell 完成一次查证并落盘 eval.md，满足 AP19 通过判定。

### 是否直接调用了 mcp__*/run_mcp/裸 npx mcporter call

否，仅通过白名单 shell wrapper（`bash harness/mcp-bridge/bin/mcp-browser ...`）调用 contract 声明的 `--bridge-check` / `playwright.browser_navigate` / `playwright.browser_snapshot` 三条命令；未直接调用任何 `mcp__*` 工具、未使用 `run_mcp`、未使用裸 `npx mcporter call`。
